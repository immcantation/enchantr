...
if (!is.null(params$outname)) {
    output_fn <- paste0(params$outname,"_cont-flag.tsv.gz")
} else {
    output_fn <- sub("\\.tsv(\\.gz)?$", "_cont-flag.tsv.gz", basename(params$input))
}

if (!is.null(params$log)) {
    log_fn <- paste0(params$log,".txt")
} else {
    log_fn <- sub("_cont-flag.tsv.gz$", "_command_log.txt", basename(output_fn))
}
