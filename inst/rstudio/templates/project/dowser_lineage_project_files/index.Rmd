...
```{r}
h <- grepl("[hHBbDc]", db[['locus']]) #IGH, TRB, TRD
if (sum(h)>0) {
    ...
    minseq_pass <- db %>%
        group_by(clone_id) %>%
        mutate(clone_size=n()) %>%
        mutate(minseq_pass=clone_size>=params$minseq) %>%
        pull(minseq_pass)
    if (sum(minseq_pass) == 0 ) {
        warning("No clones with at least ",params$minseq, " (`minseq`) sequences")
    } else {
        clones <- formatClones(db,
                               traits = intersect(c("c_call"),colnames(db)),
                               # num_fields=intersect(c("duplicate_count", "consensus_count"),colnames(db)), ## TODO: check not all NA
                               chain="H",
                               locus='locus',
                               heavy=heavy[[1]],
                               minseq=params$minseq, nproc=params$nproc) 
        trees <- getTrees(clones, build="igphyml", nproc=params$nproc)
