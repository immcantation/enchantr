...
if (sum(heavy_chains, na.rm=T)==0){
  stop("No heavy chains detected. Heavy chains are required for clonal inference.")
}
...
if (!is.null(params$outname)) {
    output_fn <- paste0(params$outname,"_clone-pass.tsv.gz")
} else {
    output_fn <- sub("\\.tsv(\\.gz)?$", "_clone-pass.tsv.gz", basename(params$input))
}

if (!is.null(params$log)) {
    log_fn <- file.path(params$outdir,paste0(params$log,".txt"))
} else {
    log_fn <- file.path(params$outdir,sub("_clone-pass.tsv.gz$", "_command_log.txt", basename(output_fn)))
}
```
