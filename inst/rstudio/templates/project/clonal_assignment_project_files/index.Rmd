...
<div style="margin-top: -10px; font-size: 15px; color: gray;">
The clone sizes table is available <a href=tables/clone_sizes_table.tsv target="_blank">here</a>. With real datasets, most clones have a size of one (one sequence). Straight sequence count as a measure of the size of the clones is not the best measure for colonal size comparison between samples due to possible disproportionate sampling. That's why we also provide clonal abundance. You can find the Clonal abundance section below. 
</div>
...
<div style="margin-top: -10px; font-size: 16px; color: gray;">
Mutation counts are determined by comparing a reference sequence to the input sequences. Function `observedMutations` from package <a href="https://shazam.readthedocs.io/en/stable/" target="_blank">SHazaM</a> is used to calculate the observed number of mutations for each sequence. 
</div>
