...
Expected columns include `v_call`, `j_call`, `sequence_alignment`, and `junction_length`.
Only productive, in-frame sequences without stop codons are retained to avoid artefacts.
The repertoire is then grouped by germline allele to prepare for per-allele analysis.
...
Using the same reference ensures positional numbering and mutation counts remain consistent with the upstream alignment step.
Expected: each `v_call` present in the repertoire should have a corresponding entry in the germline database.
