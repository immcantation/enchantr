...
<div class="introduction_paragraph">
<strong>Limitation:</strong> This report currently supports immunoglobulin (IG) loci only. It does not work for T-cell receptor (TR) loci.
</div>
...
if (is.character(pos_range)) {
   if (grepl(":", pos_range)) {
      ...
      warning("Couldn't parse pos_range; reverting to default 1:312")
      pos_range <- 1:312
   }
}
...
cols_to_read <- unique(c("sequence_id", "productive",
                         ...
                         params$junction, params$junction_length, "v_germline_end",
...
if (any(grepl("^TR", db[["locus"]]))) {
  stop("The novel allele inference report supports IG loci only; TR loci are not supported.")
}
...
if (!is.null(params$outname)) {
  ...
  evidence_output_fn <- paste0(basename(params$outname), "_novel_allele_evidence.rda")
} else {
  output_fn <- sub("\\.tsv(\\.gz)?$", "_novel-pass.tsv", basename(params$input))
  report_output_fn <- sub("\\.tsv(\\.gz)?$", "_novel_report.tsv", basename(params$input))
  evidence_output_fn <- sub("\\.tsv(\\.gz)?$", "_novel_allele_evidence.rda", basename(params$input))
}
...
This report is produced for subject `r paste(unique(db$subject_id), collapse=", ")` using the samples `r paste(unique(db$sample_id), collapse=", ")`.
...
Verify that at least one allele in the germline database has enough supporting sequences to be evaluated. Alleles with fewer than `r params$germline_min` supporting sequences are excluded from the TIgGER novel-allele inference step.

```{r allele-frequency-distribution, results='asis'}
allele_groups <- lapply(names(germline_db), function(allele) {
  grep(allele, db[[params$v_call]], fixed = TRUE)
})
names(allele_groups) <- names(germline_db)

allele_frequency <- vapply(allele_groups, length, integer(1))
eligible_alleles <- allele_frequency >= params$germline_min
kept_alleles <- sum(eligible_alleles)
can_run_inference <- kept_alleles > 0
has_novel <- FALSE

cat(paste0("- Germline alleles evaluated: ", length(germline_db), "\n"))
cat(paste0("- Alleles meeting `germline_min` (", params$germline_min, "): ", kept_alleles, "\n"))

if (!can_run_inference) {
  cat("\nNo alleles in the germline database have enough supporting sequences. Novel allele inference is skipped for this report.\n")
}
```
...
Positions with a disproportionately high baseline mutation frequency (high y-intercept) are flagged as possible novel alleles (polymorphisms).
...
- `pos_range`: limits the region analyzed to reliably aligned V-segment positions, avoiding false SNPs near the CDR3 junction. It is applied to heavy-chain genes, and to light-chain genes when `auto_pos_range` is off.

- `auto_pos_range`: for light chains only, derive each V gene's positional range from the data (the 95th percentile of `v_germline_end`) instead of using the fixed `pos_range`. This adapts to the variable 3' end of light-chain V genes and reduces false novel SNPs there; it has no effect on heavy chains. Heavy and light chains are inferred separately, so a repertoire mixing both loci is handled correctly.

*Expected*: only a small fraction of alleles show candidate polymorphic sites; most known alleles return no novel calls.

```{r infer-novel-alleles, eval=can_run_inference}

# Infer novel alleles, splitting heavy/light chains so each uses the appropriate
# positional-range strategy (see enchantr::infer_novel_alleles). heavy_chains is
# computed here because `db` has been filtered/reordered since it was read in.
heavy_chains <- isHeavyChain(db[["locus"]])
novel <- infer_novel_alleles(db, germline_db, heavy_chains, params, pos_range, mut_range)

has_novel <- is.data.frame(novel) && nrow(novel) > 0
```
...
has_novel <- !inherits(novel, "try-error") && nrow(novel) > 0
```
...
```{r build-reference, eval=has_novel}
...
# Plot Novel Alleles Evidence

<div class="introduction_paragraph">
Plots showing evidence for each inferred novel allele. For every candidate, the plot summarizes the supporting sequence evidence across positions, as reported by TIgGER.
</div>

```{r plot-novel-alleles, eval=has_novel, results='asis', echo=FALSE}
novel_allele_figures <- list()

for (i in seq_len(nrow(novel))) {
   plot_env <- new.env()
   plot_env$allele <- novel$polymorphism_call[i]
   plot_env$p_obj <- plotNovel(db, novel[i,])
   plot_env$ix <- i
   ...
   novel_allele_figures[[plot_env$allele]] <- plot_env$p_obj
}
```

# Output files

```{r save-inference, eval=has_novel, results='asis'}
# Save novel report
...
```{r save-evidence, eval=has_novel, results='asis'}
save(
  novel,
  novel_allele_figures,
  file = file.path(params$outdir, evidence_output_fn)
)
cat("Novel allele evidence: [", evidence_output_fn, "](", evidence_output_fn, ") - R data file with novel allele table and figure objects\n")
```

```{r save-germline, results='asis'}
# create the germline output directory
germline_output_dir <- file.path(params$outdir, file.path("db_novel", species, "vdj"))
...
invisible(file.copy(list.files(file.path(imgt_db, species, "vdj"), full.names = TRUE),
          germline_output_dir,
          recursive = TRUE))
...
if (!can_run_inference) {
  cat(paste0("\nSKIP> No germline alleles met germline_min=", params$germline_min), file = log_fn, append = TRUE)
}
if (exists("novel") && !inherits(novel, "try-error") && nrow(novel) > 0) {
  cat(paste0("\nOUTPUT> ", report_output_fn), file = log_fn, append = TRUE)
  cat(paste0("\nOUTPUT_EVIDENCE> ", evidence_output_fn), file = log_fn, append = TRUE)
