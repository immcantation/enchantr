...
if(params$collapse_filter_threshold > 1){
  print(paste0("Filter out sequence with less than ", params$collapse_filter_threshold, " ", params$collapse_count_colname,"."))
...
if (!is.null(params$outname)) {
   output_fn <- paste0(params$outname,"_collapse-pass.tsv.gz")
} else {
   output_fn <- sub("\\.tsv(\\.gz)?$", "_collapse-pass.tsv.gz", basename(params$input))
}

if (!is.null(params$log)) {
   log_fn <- paste0(params$log,".txt")
} else {
   log_fn <- sub("_collapse-pass.tsv.gz$", "_command_log.txt", basename(output_fn))
}
...
# Check whether group_db is empty with only sample_id because no sequence left after collapsing and filtering. If so, only keep the header.
    if (nrow(group_db)==1){
      if (is.na(group_db$sequence_alignment)) {
      group_db <- group_db[0,]}
    }
