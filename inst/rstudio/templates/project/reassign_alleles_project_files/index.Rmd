...
if (!all(is.na(references))) {
  # Reassign per locus so each repertoire is only compared against its own
  # locus' alleles, then recombine in the original row order.
  db <- reassign_loci(db, loci, segments_to_reassign, references)
}
```
...
if (!is.null(params$outname)) {
  output_fn <- paste0(params$outname, "_reassign-pass.tsv.gz")
} else {
  output_fn <- sub("\\.tsv(\\.gz)?$", "_reassign-pass.tsv.gz", basename(params$input))
}

if (!is.null(params$log)) {
  log_fn <- file.path(params$outdir, paste0(params$log, ".txt"))
} else {
  log_fn <- file.path(params$outdir, sub("_reassign-pass.tsv.gz$", "_command_log.txt", basename(output_fn)))
}
```
