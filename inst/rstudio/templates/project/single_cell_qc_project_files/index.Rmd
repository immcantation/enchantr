...
# Read db file
db <- bind_rows(sapply(db_files, read_rearrangement, simplify = F, USE.NAMES = T), .id="input_file")
...
input_summary <- db %>%
    group_by(!!!rlang::syms(unique(c("sample_id", params$outputby, "locus")))) %>%
    summarize(n=n()) %>%
...
db <- db[pass,] %>%
    mutate(cell_id = paste(cell_id,match(sample_id,sample_ids),sep="-"),
           sequence_id = paste(sequence_id,match(sample_id,sample_ids),sep="-"))
