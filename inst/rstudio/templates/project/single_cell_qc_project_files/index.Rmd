...
```{r}
db <- findSingleCellDuplicates(db, group="sample_id", cell="cell_id", seq="sequence_alignment")
ggplot(db, aes(x=sample_id, fill=sc_duplicate)) +
    geom_bar()
```
...
write_rearrangement(db[pass,] %>% select(-sc_duplicate), file=output_fn)
```
