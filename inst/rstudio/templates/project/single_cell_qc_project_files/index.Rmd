...
if (dups_found) {
  ...
  fail <- dups[["dups"]][["sc_duplicate_cell"]] == TRUE
} else {
  pass <- rep(TRUE, nrow(db))
}

if (!is.null(params$outname)) {
  output_fn <- paste0(params$outname, "_scqc-pass.tsv.gz")
  failed_fn <- paste0(params$outname, "_scqc-fail.tsv.gz")
} else {
  output_fn <- sub("\\.tsv(\\.gz)?$", "_scqc-pass.tsv.gz", basename(params$input))
  failed_fn <- sub("\\.tsv(\\.gz)?$", "_scqc-fail.tsv.gz", basename(params$input))
}

if (!is.null(params$log)) {
  log_fn <- paste0(params$log, ".txt")
} else {
  log_fn <- sub("_scqc-pass.tsv.gz$", "_command_log.txt", basename(output_fn))
}
```
...
```{r dbfailed-save, evaluate=dups_found, results="asis", echo=FALSE}
# Save the failed sequences into a separate file if duplicates found
failed_db <- db[!pass, , drop=FALSE]
write_rearrangement(failed_db, file = file.path(params$outdir, "repertoires", failed_fn))
cat("\n\nSequences that were removed due to contamination are saved in [",
    failed_fn, "](", file.path("repertoires", failed_fn), ").\n\n", sep = "")
```
