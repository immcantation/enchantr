...
```{r p_sc_duplicates, fig.cap=p_sc_duplicates$enchantr$html_caption}
...
dups_summary <- dups[['dups']] %>%
    group_by(sample_id) %>%
    summarize(duplicates=sum(sc_duplicate),
              not_duplicates=sum(!sc_duplicate)) %>%
    rowwise() %>%
...
```{r p-overlap-matrix, eval=TRUE,  fig.cap=p_overlap_matrix$enchantr$html_caption}
