...
dups$dups <- dups$dups %>%
    ...
    mutate(sc_duplicate_cell = if_else(any(sc_duplicate), T, F))

num_cells_with_dups <- dups$dups %>%
    filter(sc_duplicate_cell) %>%
    select(sample_id, cell_id) %>%
    distinct() %>%
    nrow()

cat(
    sum(dups$dups$sc_duplicate), " duplicated sequences detected. Removing ",
...
tab_dups_summary <- eetable(
    ...
    caption = paste0("Summary of duplicated sequences: sequences belonging to cells with
                     suspicious sequences (sequences that have the 
