...
caption_text <- paste0("Summary of input data. Number of sequences per sample and 
...
```{r sc-duplicates-zero, results="asis", eval=(nrow(dups[["dups"]]) == 0), echo=FALSE}
if (nrow(dups[["dups"]]) == 0) {
  cat("No suspicious overlaps detected.\n\n")
}
```

```{r sc-duplicates-summary, results="asis", eval=(nrow(dups[["dups"]]) > 0)}
...
tab_dups_summary <- eetable(
    ...
    caption = paste0("Summary of duplicated sequences: sequences that have the 
                     same cell_id and sequence across samples. The percent represents 
                     the number of duplicated sequences over the total of sequences in the sample."))
...
```
```{r echo=FALSE, results="asis", eval=(nrow(dups[["dups"]]) > 0)}
print_table_caption(tag = "dups_summary", text = tab_dups_summary$caption)
```

## Stacked bar plot

```{r p-sc-duplicates, fig.cap=p_sc_duplicates$enchantr$html_caption, results="asis", eval=(nrow(dups[["dups"]]) > 0)}
...
if (nrow(dup_count) > 0) {
    ...
    overlap_summary_tab <- eetable(
        overlap_summary,
        caption=paste0("Summary of duplicated cells: cells that have sequences with a combination of cell_id and sequence found in multiple samples.")
    )
...
```
```{r echo=FALSE, results="asis", eval=(nrow(dup_count) > 0) }
print_table_caption(tag = "overlap_summary", text = overlap_summary_tab$caption)
```

```{r p-overlap-matrix, eval=TRUE, fig.cap=caption, eval=(nrow(dup_count) > 0) }
