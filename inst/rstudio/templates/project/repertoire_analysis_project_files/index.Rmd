...
<div class="introduction_paragraph">
A table summarizing the number of clones in each sample and  their frequency in the repertoire is available <a href=tables/clone_sizes_table.tsv target="_blank">here</a>. In real datasets, most clones contain only one unique sequence as they represent naive cells that have not undergone clonal expansion (also termed singletons). Straight sequence count as a measure of the size of the clones is not the best measure for clonal size comparison between samples due to possible disproportionate sampling. The clonal abundance and diversity computations (see sections below) address this issue by bootstrapping (i.e. re-sampling multiple times) the sample sequences to the same number of sequences so that clonal abundance and diversity can be compared across samples.
</div>
...
<div class="introduction_paragraph">
Clonal abundance is the size of each clone as a fraction of the entire repertoire. To correct for the different number of sequences in each of the samples, Function `estimateAbundance` from package <a href="https://alakazam.readthedocs.io/en/stable/" target="_blank">Alakazam</a> estimates the clonal abundance distribution along with confidence intervals on these clone sizes using bootstrapping (i.e. selecting the same number of sequences to compare for all samples and repeating this selection multiple times to compute the mean and confidence intervals on the clonal abundance). `r params$nboot` random bootstrap samples were taken, with size the number of sequences in the sample with less sequences (N).
</div>
...
In clonal overlap without bootstrapping, overlapping clones are counted using all sequences from each paired samples.
In clonal overlap with bootstrapping, the average number of overlapping clones is calculated across bootstrap resamples, which corrects for differences in sequencing depth among samples.
</div>
...
text_size <- floor(min(3,20/max_n_samples)) # No text in heatmap cell if max_n_samples > 20
...
caption<- "Clonal Overlap Among Samples"

cat("## Clonal overlap without bootstrapping\n")

if (text_size>0){
  cat("\n Clonal overlap among samples. Numbers in each cell are the number of clones shared by the x-axis sample and the y-axis sample. Percentages indicate the fraction of shared clones from the total number of clones in the y-axis sample. Each cell is colored by percentage of clonal overlap. \n")
} else{
    cat("\n Clonal overlap among samples. Each cell is colored by percentage of clonal overlap, which is the fraction of shared clones in the y-axis sample. \n")
}
...
plot_list <- lapply(calculate_overlap_list, function(item) {
  ...
  clonal_overlap_p <- clonal_overlap_p + ggtitle(paste0("Clonal Overlap Heatmap(",params$cloneby, ": ", item,")"))

  eeplot(
    clonal_overlap_p,
    outdir = params$outdir,
    file   = paste0("Clonal_Overlap_", params$cloneby, "_", item),
    caption = caption
  )
...
cat(paste0("\n Average clonal overlap among samples over all bootstrapping repetitions.", "Bootstrapping drew ",unique(a@n), " sequences from each sample each time for ", a@nboot, " bootstrapping repetitions.", "Each cell is colored by the percentage of the average clonal overlap over the bootstrapping repetitions in the y-axis sample."))
...
plot_list <- lapply(calculate_overlap_list, function(item) {
  ...
  bootstrapping_clonal_overlap_p <- bootstrapping_clonal_overlap_p + ggtitle(paste0("Bootstrapping Clonal Overlap Heatmap(",params$cloneby, ": ", item,")"))

  eeplot(
    bootstrapping_clonal_overlap_p,
    outdir = params$outdir,
    file   = paste0("Bootstrapping_Clonal_Overlap_", params$cloneby, "_", item),
    caption = caption
  )
...
if (!is.null(params$outname)) {
    output_fn <- paste0(params$outname,"_with_mutation_freq.tsv")
} else {
    output_fn <- sub(".tsv$", "_with_mutation_freq.tsv", basename(params$input))
}

if (!is.null(params$log)) {
    log_fn <- file.path(params$outdir,paste0(params$log,".txt"))
} else {
    log_fn <- file.path(params$outdir,sub("_with_mutation_freq.tsv$", "_command_log.txt", basename(output_fn)))
}
```
