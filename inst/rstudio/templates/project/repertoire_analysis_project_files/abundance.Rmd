...
a@abundance <- a@abundance %>%
   left_join( db_keep_for_abundance %>% 
...
subject_levels <- str_sort(unique(a@abundance$subject_id), numeric = TRUE)
sample_levels  <- str_sort(unique(a@abundance$sample_id),  numeric = TRUE)
a@abundance <- a@abundance %>%
  mutate(
...
```{r abundanceSample, eval=TRUE, fig.width=getPrettyWidth(length(unique(db_keep_for_abundance[['sample_id']])), ncols=3), fig.height=getPrettyHeight(length(unique(db_keep_for_abundance[['sample_id']])),ncols=3), fig.cap=caption}
...
abundanceSample <- eeplot(abundanceSample, 
                     ...
                     caption=paste0("Clonal abundance plot per `sample_id`. Clonal abundance was calculated with a sample of N=",
                      ...
                      " sequences with ",
                      a@nboot,
                      " boostrapping repetitions. The y-axis shows the clonal abundance (i.e., the proportion of the clone in the repertoire as a percentage. The x-axis shows the rank of each clone (from 1 to N number of clones in the sample), where the rank is sorted by clone size from larger (rank 1, left) to smaller (right).  The x-axis rank number is represented on a logarithmic scale. The shaded areas are confidence intervals of the clonal abundance over the bootstrapping repetitions."),
                     a)
abundanceSample +
...
The clonal abundance distribution can be characterized using diversity statistics. Diversity calculation is useful to determine whether there is any difference in clonal expansion or the clonal distribution between repertoires.

Diversity scores (D) are calculated using the generalized diversity index (Hill numbers), which covers many different measures of diversity in a single function with a single varying parameter, the diversity order q.

For diversity calculations, bootstrapping is also performed (i.e. selecting the same number of sequences and repeating this procedure multiple times for each sample). For this diversity computation, `r params$nboot` bootstrapping repetitions were performed.
...
* q = 2 corresponds to the Simpson Index.
</div>
...
d <- alphaDiversity(db_keep_for_abundance %>% filter(isHeavyChain(locus)), 
                    group = "sample_id", 
                    min_n=params$min_n)
...
subject_levels <- str_sort(unique(d@diversity$subject_id), numeric = TRUE)
sample_levels  <- str_sort(unique(d@diversity$sample_id),  numeric = TRUE)
d@diversity <- d@diversity %>%
  mutate(
    subject_id = factor(subject_id, levels = subject_levels),
    sample_id  = factor(sample_id,  levels = sample_levels)
...
```{r diversitySample, eval=TRUE,  fig.cap=caption, fig.width=getPrettyWidth(length(unique(db_keep_for_abundance[['sample_id']])), ncols=3), fig.height=getPrettyHeight(length(unique(db_keep_for_abundance[['sample_id']])),ncols=3)}
...
diversitySample <- eeplot(diversitySample, 
                     ...
                     caption=paste0("Clonal diversity per `sample_id`.Clonal diversity was calculated with a sample of N=",
                      ...
                      " sequences with ",
                      a@nboot,
                      " boostrapping repetitions. The diversity (D) is shown on the y-axis and the x-axis represent the diversity orders q. Lower diversity generally indicates that there expanded clones in the repertoire."),
                     d)
diversitySample +
    theme(legend.position = "none")
...
```{r div-at-q, eval=TRUE,  fig.cap=caption, fig.width=5, fig.height=3}
caption = "Diversity at different q values."
# plot duplicated cells
div_data <- d@diversity %>% dplyr::filter(q==0 | q==1 | q==2)

div_at_q_plot <- ggplot(div_data, aes(x=q, y=d, color=sample_id)) +
    geom_point() + theme_enchantr() +
    scale_x_continuous(breaks= c(0,1,2))+ 
    theme(text = element_text(family = "Helvetica"))
    
div_at_q_plot <- eeplot(div_at_q_plot, 
                     outdir=params$outdir, 
                     file=knitr::opts_current$get('label'),
                     caption= caption)
ggplotly(div_at_q_plot+theme(legend.position="right"))
caption <- div_at_q_plot$enchantr$html_caption
```
