...
The clonal abundance distribution can be characterized using diversity statistics. Diversity scores (D) can be calculated using the generalized diversity index (Hill numbers), which covers many different measures of diversity in a single function with a single varying parameter, the diversity order q.

For diversity calculations, bootstrapping is also performed (i.e. selecting the same number of sequences for each sample and repeating this procedure multiple times). For this diversity computation, `r params$nboot` bootstrapping repetitions were performed.
...
ggplotly(div_at_q+theme(legend.position="right"))
