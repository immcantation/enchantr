...
db <- db %>% shazam::observedMutations(db, 
...
```{r abundanceSample, eval=any(db$locus %in% c("IGH", "IGL", "IGK")) & "c_call" %in% colnames(db), fig.width=getPrettyWidth(length(unique(db[['sample_id']])), ncols=3), fig.height=getPrettyHeight(length(unique(db[['sample_id']])),ncols=3), fig.cap=caption}

mufreqSample <- ggplot(db, aes(x=isotype, y=mu_freq)) + 
                    geom_boxplot() +
                    facet_wrap(~ subject_id + sample_id, ncol=3)

mufreqSample <- eeplot(mufreqSample, 
                     outdir=params$outdir, 
                     file=knitr::opts_current$get('mutationfrequencySample'),
