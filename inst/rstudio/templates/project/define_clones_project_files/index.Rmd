...
if (sum(heavy_chains)>0 & nrow(db)>0) {
    ...
    if (nrow(output_groups)>0) {
        ...
        write_rearrangement(db, file=file.path("repertoires",output_fn))
    }
}
```
...
if (length(output_files==1)) {
    ...
    for (i in 1:length(output_files)) {
        cat(paste0("\nOUTPUT",i,"> ",basesname(output_files[i])), file=log_fn, append=T)
        cat(paste0("\nPASS",i,"> ",basename(output_sizes[i])), file=log_fn, append=T)
    }
}
