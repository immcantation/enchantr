...
tab <- eetable(num_clones_table_nosingle, caption = tab_caption, outdir=params$outdir)
tab$table
...
if (!is.null(params$outname)) {
    output_fn <- file.path(params$outdir,paste0(params$outname,"_clone-pass.tsv"))
} else {
    output_fn <- file.path(params$outdir,sub(".tsv$", "_clone-pass.tsv", basename(params$input)))
}
...
if (sum(heavy_chains)>0 & nrow(db)>0) {
    ...
    if (nrow(output_groups)>0) {
        for (i in 1:nrow(output_groups))  {
            ...
            cat("- [",group_db_label,"](",params$outdir,")", sep="")
        }
    } else {
