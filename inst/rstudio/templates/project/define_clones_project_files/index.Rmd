...
} else if (params$isotype_column == "cregion" ) {
    db <- db %>%
    mutate(isotype = case_when(
      grepl("IGHA", cregion) ~ "IGHA",
      grepl("IGHM", cregion) ~ "IGHM",
      grepl("IGHG", cregion) ~ "IGHG",
      grepl("IGHE", cregion) ~ "IGHE",
      grepl("IGHD", cregion) ~ "IGHD",
      .default = NA
    ))
} else {
    stop("isotype_column must be either `c_call` or `cregion`, other isotype columns are not supported.")
}
...
```{r eval=any(db$locus %in% c("IGH", "IGL", "IGK")), results='asis'}
cat("Showing mutation frequency per isotype of heavy chains only if the `c_call` or `cregion` field as well as the `germline_alignment_d_mask` are present.\n")
