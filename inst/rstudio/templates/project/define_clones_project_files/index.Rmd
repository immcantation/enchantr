...
Number of sequences loaded: `r input_size`. Number of heavy chain sequences loaded: `r sum(heavy_chains)`.
...
num_clones_table <- db %>%
   ...
   group_by(!!!rlang::syms(c("sample_id","sequences", params$cloneby, "number_of_clones"))) %>%
...
eetable(any_tissues,
    caption=paste0("Summary of expanded clones in ", paste(params$cloneby, collapse = ", "), " . Subsetting to clones of size > 1 in the cloning group (not the sample).")
) 
```
...
```{r abundanceSample, eval=TRUE,  fig.cap=caption, fig.width=6, fig.width=0.4*length(unique(db[['sample_id']])), fig.height=0.4*length(unique(db[['sample_id']]))}
# plot duplicated cells
caption <- "Clonal abundance plot."
abundanceSample <- p + facet_wrap(~ subject_id + sample_id)
...
```{r abundanceTissue, eval=TRUE, fig.width=6, fig.height=3, fig.cap=caption}
# plot duplicated cells
...
```{r overlapthrplot, fig.width=6, fig.width=0.4*length(unique(db[['sample_id']])), fig.height=0.4*length(unique(db[['sample_id']]))}
overlap_thr$p
```
