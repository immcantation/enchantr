...
if (params$isotype_column == "c_call") {
    ...
    mutate(isotype = alakazam::getGene(c_call, first=TRUE))
} else {
    db <- db %>%
...
db_isotype <- db %>% filter(!is.na(isotype), !(is.na(mu_freq)))
db_isotype$mu_freq <- as.numeric(db_isotype$mu_freq)

if (nrow(db_isotype) > 0) {
    mufreqSample <- ggplot(db_isotype, aes(x=isotype, y=mu_freq, fill=isotype)) + 
                    geom_boxplot() +
