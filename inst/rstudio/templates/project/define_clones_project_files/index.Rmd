...
```{r eval=any(db$locus %in% c("IGH")) & params$isotype_column %in% colnames(db) , results='asis'}
cat("## Sequences per isotype\n")
```

```{r input-isotype-sumary, eval=any(db$locus %in% c("IGH")) & params$isotype_column %in% colnames(db) , results='asis'}
# Create isotype column
if (params$isotype_column == "c_call") {
    db <- db %>%
    mutate(isotype = ifelse(grepl("IGH", locus), alakazam::getGene(c_call, first=TRUE), NA))
} else {
    db <- db %>%
    mutate(isotype = case_when(
      grepl("IGHA", get(!!params$isotype_column)) ~ "IGHA",
      grepl("IGHM", get(!!params$isotype_column)) ~ "IGHM",
      grepl("IGHG", get(!!params$isotype_column)) ~ "IGHG",
      grepl("IGHE", get(!!params$isotype_column)) ~ "IGHE",
      grepl("IGHD", get(!!params$isotype_column)) ~ "IGHD",
      .default = NA
    ))
}
...
```{r echo=FALSE, eval=any(db$locus %in% c("IGH")) & params$isotype_column %in% colnames(db), results='asis'}
...
```{r eval=any(db$locus %in% c("IGH", "IGL", "IGK")), results='asis'}
cat("Showing mutation frequency per isotype of heavy chains only if the `c_call` or `isotype_column` field as well as the `germline_alignment_d_mask` are present.\n")
cat("The mutation frequency per sequence is stored in the final dataframes in the `mu_freq` column.\n")
```


```{r mufreqSample, eval=any(db$locus %in% c("IGH", "IGL", "IGK")) & "isotype" %in% colnames(db), fig.width=getPrettyWidth(length(unique(db[['sample_id']])), ncols=3), fig.height=getPrettyHeight(length(unique(db[['sample_id']])),ncols=3), fig.cap=caption}
db_isotype_igh <- db %>% filter(!is.na(isotype), locus=="IGH")

if (nrow(db_isotype_igh) > 0) {
...
db_locus <- db %>% filter(!is.na(germline_alignment_d_mask))

if (nrow(db_locus) > 0) {
    mufreqSampleLocus <- ggplot(db %>% filter(!is.na(germline_alignment_d_mask)), aes(x=locus, y=mu_freq, fill=locus)) + 
                    geom_boxplot() +
                    facet_wrap(~ subject_id + sample_id, ncol=3) +
                    theme_enchantr() +
                    theme(axis.text.x = element_text(angle = 45, hjust = 1))
