...
```{r clonesize, fig.width=8, fig.height=min(16,max(4,0.7*length(unique(db[['sample_id']])))), fig.cap=caption, eval=any(isHeavyChain(db$locus))}
...
```{r clone-size-atleast2, eval=max(clone_sizes[['seq_count']])>1, fig.width=8, fig.height=min(16,max(4,0.7*length(unique(db[['sample_id']])))), fig.cap=caption}
...
for (i in 1:nrow(output_groups))  {
    group_db <- right_join(db,
       ...
       select(-input_file,-num_input_files,-input_size)
