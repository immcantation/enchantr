...
Clonal assignment performed with `scoper::hierarchicalClones`, version `r v <- packageVersion("scoper"); v`.
...
## Number of clones (incl. singletons)

```{r}
num_clones_table <- db %>%
   group_by(sample_id) %>%
   mutate(sample_size=n()) %>%
   select(!!!rlang::syms(c("sample_id","sample_size", "clone_id", params$cloneby))) %>%
   group_by(!!!rlang::syms(c("sample_id","sample_size", params$cloneby, "clone_id"))) %>%
   ...
   summarize(
      min_clone_size=round(min(clone_size),3),
      mean_clone_size=round(mean(clone_size),3),
      median_clone_size=round(median(clone_size),3),
      max_clone_size=round(max(clone_size),3)
   )
...
## Number of clones (without singletons)
...
multi_tissue <- multi_tissue %>%
   left_join(db, by=c("clone_id", params$cloneby)) %>%
   select(!!!rlang::syms(c("clone_id", "tissue", "sample_id", params$clone_by, "sequence_id"))) %>%
   group_by(!!!rlang::syms(c("clone_id", "tissue", "sample_id", params$clone_by))) %>%
   summarize(size=n())
...
```{r}
any_tissues <- db %>%
   select(!!!rlang::syms(c("sample_id", "clone_id", "tissue", params$cloneby))) %>%
   group_by(!!!rlang::syms(c("sample_id", "tissue",params$cloneby, "clone_id"))) %>%
   summarize(clone_size=n(),.groups="drop") %>%
   group_by(!!!rlang::syms(c("clone_id", "clone_size", params$cloneby))) %>%
   summarize(num_tissues=length(unique(tissue)),
             tissues=paste(sort(unique(tissue)),collapse=" & "), .groups="drop") %>%
   filter(clone_size>1 | num_tissues > 1) %>%
   group_by(!!!rlang::syms(c("tissues", params$cloneby))) %>%
   summarize(num_clones=length(unique(clone_id)))
    
eetable(any_tissues,
    caption=paste0("Summary of clones by ", paste(params$cloneby, collapse = ", "), " and tissues. Subsetting to clones of size > 1 in a sample or spanning multiple tissues (can have one sequence in each tissue)")
) 
```
