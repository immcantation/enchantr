...
db<- bind_rows(lapply(strsplit(params[['input']],",")[[1]], function(x) {
    ...
    airr <- read_rearrangement(x, aux_types=aux_cols)
    airr[['input_file']] <- basename(x)
    airr[['input_size']] <- NULL
    airr
...
```{r clone-size, fig.width=8, fig.height=max(4,0.7*length(unique(db[['sample_id']])))}
...
```{r clone-size-atleast2, eval=max(clone_sizes[['seq_count']])>1, fig.width=8, fig.height=max(4,0.7*length(unique(db[['sample_id']]))) }
...
```{r, child=c('overlap.Rmd'), eval=!as.logical(params$skip_convergence)}
```
