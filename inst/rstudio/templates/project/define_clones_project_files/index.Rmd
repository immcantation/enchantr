...
```{r}
# Read repertoire
db <- readInput(params[['input']], pattern = "pass.tsv", col_select = NULL)
...
```{r clonesize, fig.width=8, fig.height=max(4,0.7*length(unique(db[['sample_id']]))), fig.cap=caption, eval=any(isHeavyChain(db$locus))}
...
```{r clone-size-atleast2, eval=max(clone_sizes[['seq_count']])>1, fig.width=8, fig.height=max(4,0.7*length(unique(db[['sample_id']]))), fig.cap=caption, eval=any(isHeavyChain(db$locus)) }
