...
if (!is.null(params$convergence_vgene)) {
    db <- db %>% filter(v_call %in% params$convergence_vgene)
}
...
```{r overlapconvergenceplot, fig.width=min(10,max(5,0.5*length(unique(db[['sample_id']])))),  fig.height=min(50,max(5,0.5*length(unique(db[['sample_id']])))), fig.cap=caption}
