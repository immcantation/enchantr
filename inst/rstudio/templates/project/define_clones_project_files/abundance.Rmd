...
```{r abundanceTissue, eval= "tissue" %in% colnames(db), fig.width=getPrettyWidth(length(unique(db[['tissue']])), ncols=3), fig.height=getPrettyHeight(length(unique(db[['tissue']])), ncols=3), fig.cap=caption}
abundanceTissue <- p + facet_wrap(~ tissue, ncol=3)
...
```{r diversitySample, eval=TRUE,  fig.cap=caption, fig.width=7, fig.height=min(50,0.3*length(unique(db[['sample_id']])))}
...
```{r div_at_q, eval=TRUE,  fig.cap=caption, fig.width=7, fig.height=6}
# plot duplicated cells
...
div_at_q <- eeplot(div_at_q, 
                     outdir=params$outdir, 
                     file="div_at_q",
                     caption="Diversity at differet q.")
div_at_q
