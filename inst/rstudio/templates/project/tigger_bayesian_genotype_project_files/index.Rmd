...
cols_to_read <- unique(c(
  ...
  params$genotypeby, "subject_id", "sample_id"
))
...
if (params$imgt_db != "") {
  ...
  references <- dowser::readIMGT(file.path(imgt_db, params$species, "vdj"), quiet = TRUE)
} else {
  references <- NA
}
```
...
germline_output_dir <- file.path(params$outdir, "db_genotype")
dir.create(germline_output_dir, recursive = TRUE, showWarnings = FALSE)
references_dir <- file.path(imgt_db, params$species, "vdj")
# generate the personal genotyped reference
generate_genotyped_reference(
  genotypes = genotypes,
  references = references,
  output_dir = germline_output_dir,
...
cat(paste0("\nOUTPUT_REFERENCE> ", germline_output_dir), file = log_fn, append = TRUE)
```
