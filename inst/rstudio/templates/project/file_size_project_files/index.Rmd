...
if (!is.null(params$metadata)) {
    ...
    # If metadata ends up empty or with only 1 row after processing, set to NULL
    if (nrow(metadata) <= 1) {
        warning("Metadata file provided but resulted in ", nrow(metadata), 
                " row(s) after processing. Setting metadata to NULL.")
        metadata <- NULL
    }
} else {
    metadata <- NULL
...
for (i in order(ids)) {
    ...
    # Handle empty or NULL out_filename
    if (length(out_filename) == 0 || is.null(out_filename) || out_filename == "") {
        out_filename <- paste0("sample_", i)
    }
    ...
    out_filename_base <- if (grepl("\\.", out_filename)) {
        strsplit(out_filename, "\\.")[[1]][1]
    } else {
        out_filename
    }
    p <- eeplot(p,outdir=params$outdir,
           file=out_filename_base,
           caption="")
    print(p)
...
id_col <- intersect(id_col, colnames(log_data))
if (length(id_col) == 0) {
    # If neither column exists, create a default grouping column
    log_data$group_id <- "all"
    id_col <- "group_id"
} else {
    id_col <- id_col[1]
}
