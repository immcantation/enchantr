...
#' overlap <- plotDbOverlap(db, group="sample_id", features="sequence_alignment", exact=F, similarity="jaccard")
#' 
...
#' 
#' @export
plotDbOverlap <- function(db, group="sample_id", features="sequence_alignment", 
    ...
    db$GROUPS <- db %>%
        dplyr::group_by(!!rlang::sym(group)) %>% group_indices()
                ...
                sapply(1:total_ii_feature, function(seq_ii) {
                    sapply(1:total_jj_feature, function(seq_jj) {
                        ...
                        if (length(sequence_ii) > 0 & length(sequence_jj) > 0) {
                            ...
                            #d_mat[seq_ii,seq_jj] <<- alakazam::seqDist(sequence_ii, sequence_jj, dist_mat = getDNAMatrix(gap=0)) == 0
                        }
                    })
                })
