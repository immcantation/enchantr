...
validate_input_project <- function(path,...) {
    skeleton_dir <- file.path(system.file(package = "enchantr"), "rstudio",
                              "templates", "project",
                              "input_validation_project_files")
    project_dir <- path
...
validate_input <- function(input, miairr, collapseby, cloneby, reassign=TRUE) {
    # Read metadata file
    input <- read.csv(input,sep = "\t", header=TRUE, stringsAsFactors = FALSE)

    # Validate metadata file
    miairr_metadata <- read.csv(miairr, sep="\t", stringsAsFactors = FALSE)
    mandatory <- miairr_metadata[['Mandatory.BioSample.attribute']] == TRUE
    mandatory_fields <- setdiff(c(miairr_metadata[['AIRR.Formats.WG.field.name']][mandatory],
                                  "pcr_target_locus", "single_cell", "species"),
                                "organism")
    ...
    if (length(unknown_collapseby)>0) {
        stop("Unknown `collapseby` found: ",
             paste(unknown_collapseby, collapse = ","))
    } else {
        input$valid_collapseby <- TRUE
    }
    ...
    if (length(unknown_cloneby)>0) {
        stop("Unknown `cloneby` found: ",
             paste(unknown_cloneby, collapse = ","))
    } else {
    ...
    input$valid_single_cell <- input[['single_cell']] %in%
        c(TRUE, FALSE, "true", "false", "TRUE", "FALSE")
    is_single_cell <- input[['single_cell']] %in% c(TRUE, "true", "TRUE")
    ...
    input[['valid_pcr_target_locus']] <-
        tolower(input[['pcr_target_locus']]) %in%
        tolower(c("IGH", "IGI", "IGK", "IGL", "TRA", "TRB", "TRD", "TRG", "IG", "TR"))
...
get_valid_species <- function(dictionary=valid_species, words) {
    dictionary <-  bind_rows(lapply(dictionary, function(x) {data.frame("species"=x)} ),
                             .id="valid_species")
    sapply(words, function(word){
        ...
        if (nrow(valid_species)>0){
            if(length(col)>1) {
                warning("Multiple `species` mappings found for ", word, ": ",
                        paste(as.character(valid_species), collapse=","))
                NA
            } else {
